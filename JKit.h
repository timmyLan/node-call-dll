...
//PassThru library info
typedef struct _JK_PassThruInfo_
{
	char	*szVendor;			// Vendor
	char    *szName;				// Device Name
	char    *szFunctionLibrary;   // library Path
	char    *szConfigApplication; // Config Tools Path
}JK_PassThruInfo;


//PassThru library info list
typedef struct _JK_PassThruInfoList_
{
	JK_PassThruInfo* pPassThruLibs;
	unsigned int     nCount;
}JK_PassThruInfoList;

typedef struct _JKit_PassThru_DLL_
{
	HINSTANCE	hDLL;
	JK_PassThruInfo	hDllInfo;
	HANDLE		hMutex;
...
typedef struct _JKitHandle_
{
	unsigned long   hDeviceID;							//PassThru 设备句柄
	unsigned long   hChannelID;                         //PassThru 通道句柄
	JKit_PassThru_DLL* PassThruDLL;							//PassThru DLL
}JKitHandle;
...
/*
 * Function: EnumPassThruInterfaces
 * Description: 
 *		从系统注册表查找所有PassThru API 接口库
 * Parameter:
 *		[out]      registrylist      PassThru API 接口库信息列表
 *      [out]      ErrMsg            出错输出信息
 * Return: 
 *		PassThru API 接口库个数
 * Others: 
 *		NULL
 */
int JKIT_API JK_EnumPassThruInterfaces(JK_PassThruInfoList &registrylist, char *ErrMsg = NULL);


/*
 * Function: JK_ReleasePassThruInterfaces
 * Description: 
 *		释放枚举的PassThruInfo 列表
 * Parameter:
 *		[in,out]    registrylist     PassThru API 接口库信息列表
 *      [in]        ErrMsg            出错输出信息
 * Return: 
 *		NULL
 * Others: 
 *		NULL
 */
void JKIT_API JK_ReleasePassThruInterfaces(JK_PassThruInfoList &registrylist, char *ErrMsg = NULL);
 ...
 */
 JKit_PassThru_DLL* JKIT_API JK_PassThruLoadLibrary(JK_PassThruInfo *PTInfo, char *ErrMsg = NULL);
 ...
 */
 long JKIT_API JK_PassThruUnLoadLibrary(JKit_PassThru_DLL *pDLL, char *ErrMsg = NULL);
